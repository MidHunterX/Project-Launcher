...
== image:./.assets/icons/tattoo.png["Tattoo",32] Customization Examples
...
Building a library or framework? The dependencies and workflow varies
drastically without any kind of patterns due to swappable dependencies and
custom configurations. In this scenario, runner most likely detects "none" as
project type. You can write the workflow logic yourself.
...
setup_env_custom() {
    if [[ ! -d "venv" ]]; then
        setup_base_env "venv" "python -m venv venv"
        source "./venv/bin/activate"
...
setup_layout_custom() {
    ...
    create_window "Test" "pytest --no-header"
}
----

== image:./.assets/icons/property_unavailable.png["Red House",32] Known Issues & Limitations

* No live lockfile tracking beyond initial check. If any dependencies change in repo while development, it has to be handled manually. Or just delete the dependency dir and run again for a clean project.
...
== image:./.assets/icons/bank.png["Green Dollar",32] Notes

=== Passwordless Service Toggle

To make remove the toggle service password friction, you can set up the service to be passwordless in systemd:

First check where systemctl is:
...
Create a sudoer entry for the service

[source,bash]
----
sudo visudo -f /etc/sudoers.d/<service_name>
----

Add entry in the following structure:

[source,bash]
----
<username> ALL= NOPASSWD: <systemctl_path> * <service_name>
----
...
midhunter ALL= NOPASSWD: usr/bin/systemctl * postgresql.service
----
...
// Fun, interesting & cool websites, never be bored again. Hundreds of hand-picked fun websites, funny finds, free online games, cool tools and things to learn.
image::./.assets/icons/save_game.png["Floppy Disk",46,link="https://www.bored.com/",window=_blank]

You dug down this far and struck gold, here's a save file just for you. Click on it.
